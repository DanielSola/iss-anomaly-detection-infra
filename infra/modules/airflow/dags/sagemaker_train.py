...
def get_model_key(**kwargs):
    ...
    train_params = kwargs['ti'].xcom_pull(task_ids='train_sagemaker_model', key='return_value')

    if not train_params:
        raise ValueError("Train Params not found in XCom")

    job_name = train_params['Training']['TrainingJobName']
...
with DAG(
    ...
    task_train_model = SageMakerTrainingOperator(
        ...
        do_xcom_push=True,  # Ensure the output is pushed to XCom
    )
